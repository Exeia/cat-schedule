<?php
  ...
  $data = "host=".$hostname." port=5432 dbname=".$dbname." user=postgres"; 
  $GLOBALS['db'] = pg_connect($data);

  function get_dog($handle)
  {
  	$query = "select * from users where handle = $1";
  	$result = pg_query_params($GLOBALS['db'], $query, array($handle));
  ...
  function add_dogs($handle, $mcecs, $real_name, $pdx_name, $is_admin)
  {
    ...
    $result = pg_query_params($GLOBALS['db'], $query, array($handle, $mcecs, $real_name, $pdx_name,$is_admin));
    return $result;
  ...
  function get_user_shifts($handle,$date_from, $date_to)
  {
  	...
  	$result = pg_query_params($GLOBALS['db'], $query, array($handle, $date_from, $date_to));
  	$row = pg_fetch_all($result);
  	if (empty($row)) {
  		return array();
  	}
  ...
  function is_available($date,$size)
  {
    ...
    $result = pg_query_params($GLOBALS['db'], $query, array($date));
    $row = pg_fetch_all($result);
    if(sizeof($row) < $size){
      return true;
    }
  ...
  function get_user_shift_preference($user_id, $week)
  {
    ...
    $result = pg_query_params($GLOBALS['db'], $query, array($user_id, $week));
    $row = pg_fetch_all($result);
    if (empty($row)) {
      return array();
    }
  ...
  function assign_shift($handle, $start_shift,$end_shift, $day_of_week)
  {
    ...
    $result = pg_query_params($GLOBALS['db'], $query, array($handle, $shift));
    return $result;

  }
  function drop_shift($user_id, $date){
    $query = "DELETE from shifts where user_id =$1 and date = $2 ";
    $result = pg_query_params($GLOBALS['db'], $query, array($user_id, $date));
    return $result;
  ...
  function clock_in_out($user_id,$shift_id)
  {
    ...
    $result = pg_query_params($GLOBALS['db'], $query, array($user_id, $shift_id));
    ...
    if($clockin)
    {
      ...
      $result = pg_query_params($GLOBALS['db'], $query_update, array($clockin, $shift_time, $shift_id));
      return true;
